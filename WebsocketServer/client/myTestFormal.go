...
func loginPackage() []byte {
	token := []byte("b91df4b2cbbd49409a8d1f30655a173372617211") //lizhanbin@dji.com正式环境的token
	buf := new(bytes.Buffer)
	nLen := uint16(1 + 2 + 1 + 16 + len(token) + 2)
	vl := uint16(1<<10 | nLen)
	sn := []uint8{56, 56, 56, 56, 56, 56, 56, 56, 0, 0, 0, 0, 0, 0, 0, 0}
...
func initData(frameindex uint8, n int, start uint64) []byte {
	...
	var data = []interface{}{
		...
		[]uint8{56, 56, 56, 56, 56, 56, 56, 56, 0, 0, 0, 0, 0, 0, 0, 0},
		uint8(0x1),
		uint8(0x0),
		uint16(200),
		float32(1.111111),
