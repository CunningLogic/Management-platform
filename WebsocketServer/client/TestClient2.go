...
func loginPackage() []byte {
	token := []byte("4dad6cc4a0e74a13a89477c04243434065216257")
	buf := new(bytes.Buffer)
	nLen := uint16(1 + 2 + 1 + 16 + len(token) + 2)
	vl := uint16(1<<10 | nLen)
	sn := []uint8{49, 50, 51, 52, 53, 54, 55, 56, 57, 57, 0, 0, 0, 0, 0, 0}
...
func initData(frameindex uint8, n int, start uint64) []byte {
	...
	var data = []interface{}{
		...
		uint8(0x05),
		t,
		float64(113.166842 + 0.0002*float64(n)),
		float64(23.24564 + 0.0002*float64(n)),
		float32(88.33333),
		//[]uint8{65, 66, 67, 68, 69, 70, 71, 72, 73, 73, 74, 75, 76, 77, 79, 78},
		[]uint8{49, 50, 51, 52, 53, 54, 55, 56, 57, 57, 0, 0, 0, 0, 0, 0},
		uint8(0x1),
		uint8(0x0),
		uint16(0x2),
		float32(1.111111),
...
func returnLock(conn net.Conn, info PackageHeadInfo) {
	/*
	   manage package
	   login package
	   head  | version | cmd_id | FlyID  | CMD    | CRC16
	   1BYTE | 2BYTE   | 1BYTE  | 16BYTE | 1BYTE  | 2BYTE
	*/
...
func Reader(conn net.Conn) {
	...
	for {
		...
		case 0x08:
			fmt.Println("lock time period recieve...")
		}
		conn.Read(ack) //把头部以外的数据读取掉
		conn.Read(CRC16)
...
func Sender(service string, i, count, sleepCnt int) {
	...
	for n := 0; n < count-1; n++ {
		...
		//fmt.Println(d)
		//fmt.Println(w)
		err := SafeWrite(conn, w)
		if err != nil {
			fmt.Println("disconnet...", err)
			conn.Close()
			break
		}
		//conn.Read(buf[0:])
