...
func loginPackage() []byte {
	token := []byte("c7d731a68c0542f6a9dcdaa71322631d82639600") //316892736@qq.com正式环境的token
	buf := new(bytes.Buffer)
	nLen := uint16(1 + 2 + 1 + 16 + len(token) + 2)
	vl := uint16(1<<10 | nLen)
	sn := []uint8{57, 56, 55, 54, 53, 52, 51, 50, 49, 0, 0, 0, 0, 0, 0, 0}
...
func initData(frameindex uint8, n int, start uint64) []byte {
	...
	var data = []interface{}{
		...
		[]uint8{57, 56, 55, 54, 53, 52, 51, 50, 49, 0, 0, 0, 0, 0, 0, 0},
		uint8(0x1),
		uint8(0x0),
		uint16(200),
		float32(1.111111),
