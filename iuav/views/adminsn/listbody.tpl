...
<div class="site-login">
    ...
    <p>sn和代理列表      <a href='/adminsn/addbody/'>增加sn和代理关系</a> &nbsp;&nbsp;  <a href="/adminsn/list">sn</a>     </p>

    <p>
        <form action="/adminsn/listbody" method="post" >
        <input type="hidden" name="_csrf" value="{$csrftoken}">
        <div>&nbsp;整机序列号&nbsp;&nbsp;<input type="text" value="{$body_code|escape:"html"}" id="body_code" name="body_code" style="display:inline;">&nbsp;&nbsp;&nbsp;硬件id&nbsp;&nbsp;<input type="text" value="{$hardware_id|escape:"html"}" id="hardware_id" name="hardware_id" style="display:inline;">&nbsp;&nbsp;代理名称&nbsp;&nbsp;<input type="text" value="{$agentname|escape:"html"}" id="agentname" name="agentname" style="display:inline;">&nbsp;&nbsp;代理code&nbsp;&nbsp;<input type="text" value="{$code|escape:"html"}" id="code" name="code" style="display:inline;">&nbsp;&nbsp;<input type="submit" style="display:inline;" value="查询" > &nbsp;</div> 
       </form>
    </p>
      ...
      <table cellspacing="10" cellpadding="10" border="1" width="100%">
        ...
        <tbody id="tbody">
           ...
           <tr>
            ...
            <td>{$user.code|escape:"html"}</td>
            <td>{$user.created_at|escape:"html"}</td>
            <td>{$user.type|escape:"html"}</td>
            <td>{$user.operator|escape:"html"}</td>  
             {if $user.sninfo }
            <td>{$user.sninfo.0.activation|escape:"html"}</td>           
            {else}
            <td></td>
            {/if}

            <td><a href='/adminsn/addbody/?id={$user.id}'>修改</a> &nbsp;&nbsp;</td>            
